Порядок установки и настройки модуля оплаты при помощи системы MainPay.ru :
...
2.  В группе "Загрузить файл пакета" нажимаем кнопку "Обзор" и на компьютере
    выбираем архив в модулем mod_mainpay.zip. После этого нажимаем кнопку
...
9.  Переходим в раздел меню "Расширения -> Менеджер модулей" и в списке
    установленных модулей кликаем по ссылке "MainPay payment gateway".

10. Настраиваем параметры модуля mod_mainpay:
...
11. Далее переходим к созданию сервиса на сайте MainPay.ru по методике, описанной
    здесь: http://wiki.mainpay.ru/doku.php?id=a1_lite#%D1%81%D0%BE%D0%B7%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5_%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81%D0%B0

12. При настройке сервиса значения полей "URL скрипта обработчика на Вашем сайте",
    "URL страницы успешной покупки" и "URL страницы ошибки" берем из
    настроек модуля в (см. п. 10) в группе "Параметры модуля".

13. Возвращаемся на свой сайт и в окне настройки параметров модуля mod_mainpay
    ...
    MainPay.ru) и "Ключ платежа" (используется значение параметра "key" из
    сгенерированной платежной формы на сайте MainPay.ru).
    После этого сохраняем конфигурацию модуля mod_mainpay.

14. Теперь на вашем сайте в меню "Main menu" появляется ссылка на форму
    оплаты товаров или услуг при помощи системы MainPay.

15. Примеры РЕАКЦИИ ВАШЕГО САЙТА на оплату с помощью данного модуля находятся
    в соответствующих файлах внутри каталога с модулем:
    - в файле process.php находится процедура обработки запросов биллинга
      после выполнения оплаты на сайте MainPay.ru. Она позволяет производить
